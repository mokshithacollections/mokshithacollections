...
function openWishlistModal(product, heartEl) {
    ...
    variants.forEach(v => {
        ...
        sw.className = 'sw';
        paintVariantSwatch(sw, v);

        const ci = document.createElement('span');
        ci.className = 'ci';
        const c = document.createElement('span'); c.className = 'c'; c.textContent = v.color || 'Variant';
        // Just the size; whether it's saved is shown by the highlighted (active) chip.
        const m = document.createElement('span'); m.className = 'm'; m.textContent = v.size || '';
...
function injectVariantModalStyles() {
    ...
    .vchip { display: inline-flex; align-items: center; gap: 11px; padding: 8px 16px 8px 8px;
        border: 2px solid #e5e7eb; border-radius: 14px; background: #fff; cursor: pointer; font-family: inherit; transition: all .15s; }
    ...
    .vchip.disabled { opacity: .5; cursor: not-allowed; }
    /* Square thumbnail of the variant's first image (colour fill as fallback). */
    .vchip .sw { width: 52px; height: 52px; border-radius: 10px; background: #cbd5e1 center/cover no-repeat;
        border: 1px solid rgba(0,0,0,0.12); flex-shrink: 0; }
    .vchip .ci { display: flex; flex-direction: column; line-height: 1.25; text-align: left; }
    .vchip .ci .c { font-weight: 600; font-size: 0.95rem; color: #1f2937; }
    .vchip .ci .m { font-size: 0.8rem; color: #6b7280; margin-top: 2px; }
...
// First image of a variant (used as the chip thumbnail); '' when it has none.
function variantFirstImage(v) {
    return (v && v.images && v.images.length && v.images[0].imageUrl) ? v.images[0].imageUrl : '';
}

// Paint a chip swatch: the variant's first image, or its colour as a solid fill.
function paintVariantSwatch(sw, v) {
    const img = variantFirstImage(v);
    if (img) {
        sw.style.backgroundImage = 'url("' + img + '")';
    } else {
...
function openVariantModal(product, originBtn) {
    ...
    variants.forEach(v => {
        ...
        // Show the size only; colour name is its own line, stock is conveyed by
        // the dimmed/disabled chip rather than a "N in stock" label.
        const meta = out ? 'Out of stock' : (v.size || '');
